...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Canada Government Procurement');
}
...
const tools: McpToolExport['tools'] = [
  {
    ...
    summary: 'Open Government of Canada procurement opportunities, from the official CanadaBuys feed.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
  ...
  },
  {
    ...
    summary: 'Government of Canada contract awards — who won what — from the official CanadaBuys feed.',
    inputSchema: {
      type: 'object',
      properties: {
        query: {
...
function matches(query: string, fields: (string | null)[]): boolean {
  // Join all field values; require every word in the query to appear somewhere
  // in the combined text. This handles multi-word queries like "IT consulting
  // services" that often don't appear as an exact phrase in the CSV titles.
  const combined = fields.filter((f): f is string => typeof f === 'string').join(' ').toLowerCase();
  return query
    .toLowerCase()
    .split(/\s+/)
    .filter(Boolean)
